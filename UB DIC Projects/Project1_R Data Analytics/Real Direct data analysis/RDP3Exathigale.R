...
bk1 <- read.xls("rollingsales_brooklyn.xls",pattern="BOROUGH",perl = perl)
...
bk$BoroughName[bk$borough == 3] <- "Brooklyn"
bk$BoroughName[bk$borough == 2] <- "Bronx"
bk$BoroughName[bk$borough == 1] <- "Manhattan"
bk$BoroughName[bk$borough == 4] <- "Queens"
bk$BoroughName[bk$borough == 5] <- "Staten Island"
bk$BoroughName <- factor(bk$BoroughName)
...
ggplot(temp_data,aes(x=year_built,y=avg_sale_price)) +geom_bar(stat="identity") + coord_flip() +ggtitle("Year Built vs Average Sale Price")
rm(temp_data)
# Yearly Sales
...
ggplot(bk_yearsales,aes(x=BoroughName)) +geom_bar() +ggtitle("Sales by Borough")
# Average Price by Neighborhood
temp_data <- sqldf('select BoroughName,avg(sale_price_n) AS avg_sale_price from bk_yearsales group by BoroughName')
ggplot(temp_data,aes(x=BoroughName,y=avg_sale_price)) +geom_bar(stat="identity") + coord_flip() + ggtitle("Borough vs Average Sale Price")
rm(temp_data)
...
ggplot(temp_data,aes(x=sale_year,y=avg_sale_price)) +geom_bar(stat="identity",aes(fill=temp_data$sale_year)) + ggtitle("Year vs Average Sale Price")
rm(temp_data)
# Zip Code Sales
...
ggplot(temp_data,aes(x=zip_code,y=avg_sale_price)) +geom_bar(stat="identity") + ggtitle("Zipcode vs Avg Sale Price")
rm(temp_data)
# Neighbourhood vs Borough
ggplot(subset(bk_yearsales,!is.na(building.class.category) && !is.na(BoroughName)),aes(x=BoroughName,y=building.class.category)) +geom_bar(stat="identity",aes(fill=building.class.category))+ coord_flip() +ggtitle("Borough vs Category")

# Category vs Borough
ggplot(bk_yearsales,aes(x=BoroughName,y=building.class.category)) +geom_bar(stat="identity",aes(fill=building.class.category))+ggtitle("Borough vs Category")
